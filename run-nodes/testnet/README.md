...
1. Init the chain
    ...
    ```bash
    humansd init <moniker-name> --chain-id=testnet-1 --home $HOME/.humansd
    ```
...
3. Copy the genesis file to the `$HOME/.humansd/config` folder.
    ...
    Then copy the genesis file to the `$HOME/.humansd/config` folder.
    
    ```bash
    cp genesis.json $HOME/.humansd/config/genesis.json
    ```
...
4. Update persistent peers list in the configuration file `$HOME/.humansd/config/config.toml`.
    ...
    ```bash
    export PEERS=$(cat persistent_peers.txt| tr '\n' '_' | sed 's/_/,/g;s/,$//;s/^/"/;s/$/"/') && sed -i "s/persistent_peers = \"\"/persistent_peers = ${PEERS}/g" $HOME/.humansd/config/config.toml
    ```

5. Set minimum gas prices

    ```bash
    sed -i 's/minimum-gas-prices =.*/minimum-gas-prices = "0.025uheart"/g' $HOME/.humansd/config/app.toml
    ```
